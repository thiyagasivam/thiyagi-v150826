...
$slab_rates = [
    ...
    ['from' => 201, 'to' => 300, 'rate' => 4.50, 'fixed' => 45],
    ['from' => 301, 'to' => 400, 'rate' => 6.00, 'fixed' => 60],
    ['from' => 401, 'to' => PHP_INT_MAX, 'rate' => 7.00, 'fixed' => 80]
];
$tax_rate = 0.07; // 7% tax
...
if ($units > 0) {
    ...
    // Apply subsidy if selected (example: 30% off for first 100 units)
    if ($apply_subsidy && $units <= 100) {
        $subsidy_amount = min($total_energy_charge * 0.3, 150); // Max subsidy of ₹150
    }
...
?>
    ...
    <style>
        ...
        .calculator-card {
            ...
            margin: 2rem auto;
            padding: 2rem;
            border-top: 4px solid var(--ladakh-blue);
        }
        ...
        .total-box {
            background-color: var(--ladakh-light);
            border-left: 4px solid var(--ladakh-blue);
            padding: 1.25rem;
            border-radius: 8px;
            margin-top: 1.5rem;
        }
    ...
    <div class="max-w-4xl mx-auto px-4 py-8">
        ...
        <div class="calculator-card">
                    ...
                    <div>
                        <div class="flex items-center mt-6">
                            ...
                            <label class="ml-2 block text-sm text-gray-900" for="subsidyCheck">
                                <i class="bi bi-gift mr-2"></i> Apply Subsidy (30% off first 100 units)
                            </label>
                        </div>
                    </div>
                </div>
            ...
            <div class="bill-breakdown">
                <h3 class="text-xl font-semibold mb-4 ladakh-header flex items-center">
            ...
            <div class="bg-blue-50 border border-blue-200 rounded-lg p-4 mt-8 no-print">
                <h5 class="text-lg font-semibold ladakh-header flex items-center mb-2">
                    <i class="bi bi-info-circle mr-2"></i> About LPDD Rates
                </h5>
                <p class="text-gray-700 text-sm">
                    These calculations are based on sample tariff rates for Ladakh. Actual bills may vary based on 
                    official LPDD notifications. Fixed charges are slab-based and energy charges apply progressively.
                </p>
            </div>
        </div>
    </div>
...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
    ...
    <script>
        ...
        function calculateEstimate() {
            ...
            // Apply subsidy if checked (30% off first 100 units)
            if (applySubsidy && units <= 100) {
                const subsidy = Math.min(estimate * 0.3, 150);
                estimate -= subsidy;
            }
            
            // Add fixed charge and tax (7%)
            const total = (estimate + fixedCharge) * 1.07;
        ...
        document.getElementById('shareBtn').addEventListener('click', function() {
            const shareData = {
                title: 'LPDD Electricity Bill Calculator',
                text: `My LPDD bill estimate for ${unitsInput.value || '...'} units: ₹<?php echo isset($total_bill) ? number_format($total_bill, 2) : '0.00'; ?>. Calculate yours!`,
